...
void cereal_read_UbloxGnss(struct cereal_UbloxGnss *s, cereal_UbloxGnss_ptr p) {
	...
	case cereal_UbloxGnss_ionoData:
		s->ionoData.p = capn_getp(p.p, 0, 0);
		break;
	default:
		break;
	}
...
cereal_DriverMonitoring_ptr cereal_new_DriverMonitoring(struct capn_segment *s) {
	cereal_DriverMonitoring_ptr p;
	p.p = capn_new_struct(s, 32, 5);
	return p;
}
cereal_DriverMonitoring_list cereal_new_DriverMonitoring_list(struct capn_segment *s, int len) {
	cereal_DriverMonitoring_list p;
	p.p = capn_new_list(s, len, 32, 5);
	return p;
}
void cereal_read_DriverMonitoring(struct cereal_DriverMonitoring *s, cereal_DriverMonitoring_ptr p) {
	capn_resolve(&p.p);
	s->frameId = capn_read32(p.p, 0);
...
void cereal_write_DriverMonitoring(const struct cereal_DriverMonitoring *s, cereal_DriverMonitoring_ptr p) {
	capn_resolve(&p.p);
	capn_write32(p.p, 0, s->frameId);
...
void cereal_get_DriverMonitoring(struct cereal_DriverMonitoring *s, cereal_DriverMonitoring_list l, int i) {
	cereal_DriverMonitoring_ptr p;
	p.p = capn_getp(l.p, i, 0);
	cereal_read_DriverMonitoring(s, p);
}
void cereal_set_DriverMonitoring(const struct cereal_DriverMonitoring *s, cereal_DriverMonitoring_list l, int i) {
	cereal_DriverMonitoring_ptr p;
	p.p = capn_getp(l.p, i, 0);
	cereal_write_DriverMonitoring(s, p);
}
...
void cereal_read_Event(struct cereal_Event *s, cereal_Event_ptr p) {
	...
	case cereal_Event_driverMonitoring:
	...
	case cereal_Event_frontFrame:
		s->frontFrame.p = capn_getp(p.p, 0, 0);
		break;
	default:
		break;
	}
...
void cereal_write_Event(const struct cereal_Event *s, cereal_Event_ptr p) {
	...
	case cereal_Event_carParams:
	case cereal_Event_frontFrame:
		capn_setp(p.p, 0, s->frontFrame.p);
		break;
	default:
		break;
	}
